...
// Version 1.0.1: Fix incorrect function name in `merge_beats`.
...
//  MERGE BEATS
//
// See the reference solution to determinte this function's behaviour.
//
// `merge_beats` will be passed:
// - `track`, a pointer to a Track which is guaranteed to not be NULL.
// - `beats_to_merge`, the number of beats you will have to merge.
// - `merged_beats`, the number of beats you should merge into.
//
// `merge_beats` will not return a value, and should not print to stdout
// or stderr (even if beat is NULL).
...
// See the reference solution to determinte this function's behaviour.
//
// `save_track` will be passed:
// - `track`, a pointer to a Track which is guaranteed to not be NULL.
// - `name`, the name to save the track as (you are guaranteed this
//   name will be valid).
//
// `save_track` will not return a value, and should not print to stdout
// or stderr (even if beat is NULL).
...
// See the reference solution to determinte this function's behaviour.
//
// `load_track` will be passed:
// - `name`, the name to save the track as (you are guaranteed this
//   name will be valid).
//
// `load_track` should not print to stdout or stderr (even if beat is
//   NULL).
// `load_track` should return a track that is a copy of an earlier saved
// track, or NULL if there is no saved track.
