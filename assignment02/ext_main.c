...
//
// Assignment 2 21T1 COMP1511: CS Beats Extension
// main.c
//
// Version 1.0.0: Release
// Version 1.0.1: note_number should be key_number in help text.
// Version 1.1: Fix printf() to not print to stderr
//
...
// Extension -- Stage 4
#define COMMAND_MERGE_BEATS 'M'

// Extension -- Stage 5
#define COMMAND_SAVE_TRACK 'S'
#define COMMAND_LOAD_TRACK 'L'
...
// Do: Stage 4
static void do_merge_beats(State state, char *line);

// Do: Stage 5
static void do_save_track(State state, char *line);
static void do_load_track(State state, char *line);
...
static int run_command(State state, char *line) {
    if (line[0] == COMMAND_HELP) {
        ...
        return RETURN_PRINT;
    } else if (line[0] == COMMAND_MERGE_BEATS) {
        do_merge_beats(state, line);
        return RETURN_PRINT;
    } else if (line[0] == COMMAND_SAVE_TRACK) {
        do_save_track(state, line);
        return RETURN_PRINT;
    } else if (line[0] == COMMAND_LOAD_TRACK) {
        do_load_track(state, line);
        return RETURN_PRINT;
    } else if (line[0] == '\0') {
        return RETURN_SILENT;
    } else {
...
static void do_print_help() {
    ...
    printf("\n-------------------[ Extension -- Stage 4 ]----------------------\n");
    printf(""
        "  %c <beats_to_merge> <merged_beats>\n"
        "    Merge 'beats_to_merge' beats into 'merged_beats' beats.\n",
        COMMAND_MERGE_BEATS
    );
    printf("\n-------------------[ Extension -- Stage 5 ]----------------------\n");
    printf(""
        "  %c <name>\n"
        "    Save track as 'name' to a file.\n",
        COMMAND_SAVE_TRACK
    );
    printf(""
        "  %c <name>\n"
        "    Load track called 'name' from a file, and replace the current Track.\n",
        COMMAND_LOAD_TRACK
    );
...
static void do_merge_beats(State state, char *line) {
    ...
    int scanf_return = sscanf(line, "%c %d %d", &command, &beats_to_merge, &merged_beats);

    if (scanf_return != 3) {
        printf(
            "Command Invalid. The correct format is: %c <beats_to_merge> <merged_beats>.",
            command
        );
        return;
    }
    ...
    merge_beats(not_null(state->track), beats_to_merge, merged_beats);
...
static void do_save_track(State state, char *line) {
    if (strlen(line) < 3) {
        printf(
            "Command Invalid. The correct format is: %c <name>.",
            line[0]
        );
        return;
    }
    ...
    save_track(not_null(state->track), line+2);
...
static void do_load_track(State state, char *line) {
    ...
    Track t = load_track(line+2);
    if (t == NULL) {
        printf("Could not load track \"%s\"!", line+2);
        return;
    }
