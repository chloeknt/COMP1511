...
// July 2019
// Completed for lab08 by Chloe Toh z5362296
// 08/04/2021
...
void setup_pet(
    ...
    //Copy the strings for name and type into the struct
    strcpy(my_pet->name, name);
    strcpy(my_pet->type, type);
    //Declare and initialise variables for int values age and weight
    //Convert the strings to ints using atoi
    //Put these values into the struct
    int no_age = 0;
    no_age = atoi(age);
    my_pet->age = no_age;
    int no_weight = 0;
    no_weight = atoi(weight);
    my_pet->weight = no_weight;
}
