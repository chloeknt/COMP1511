...
void d_tnt_checked_explode(int day_map[SIZE][SIZE], int laser_y, int index, int tnt_radius, int new_position);

void night_map_mode(int night_map[SIZE][SIZE], int laser_y, int day_map[SIZE][SIZE], int keep_looping);
...
void n_tnt_checked_explode(int night_map[SIZE][SIZE], int laser_y, int index, int tnt_radius, int new_position);
...
int main (void) {
    ...
    while (keep_looping) {
        ...
        if (result_1 == EOF) {
            ...
            } else if (day_map[laser_y][index] > 3 && day_map[laser_y][index] < 10) {
            ...
            int one_try = d_shift_end_condition(day_map, laser_y);
...
void d_fire_laser(int day_map[SIZE][SIZE], int laser_y, int index) {
    ...
    while (tnt_check < 4 && tnt_check + index < SIZE) {
        ...
        day_map[laser_y][index + tnt_check] < 10) {
            ...
            d_tnt_checked_explode(day_map, laser_y, index, tnt_radius, new_position);
        } else {
 ...
 //Declare and initialise variables for user input 
    ...
    while (keep_looping) {
        ...
        if (result_1 == EOF) {
            ...
            break;
            //Commence the shift_end_condition function
            int one_try = n_shift_end_condition(night_map, laser_y);
            //If the last try has not already been used 
            if (one_try == 1) {
                last_try = one_try;
            }
