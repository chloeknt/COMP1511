...
//void stack_push(struct character_node *s, int item);
//int stack_pop(struct character_node *s);
...
void pad_left(struct character_node *characters, char pad_character) {
    ...
    if (characters != NULL) {
        //Pointers to the first and second node
        struct character_node *curr = characters->next;
        struct character_node *prev = characters;
        //Temporary variable for storage
        int tempvar1 = 0;
        int tempvar2 = 0;
        //Keep track of whether it is the first node
        int count = 0;
        //If there is only one node on the list
        if (curr == NULL) {
            prev->data = pad_character; 
        }
        //Loop through the list if there are multiple nodes
        while (curr != NULL) {
            //If it is the first node
            if (count == 0) {
                //Store the first node's data
                tempvar1 = prev->data;
                //Replace the data with the 
                prev->data = pad_character;
                tempvar2 = curr->data;
                curr->data = tempvar1;
                count = 1;
